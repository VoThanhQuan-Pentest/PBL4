...
- Redis phai dung named volume `redis-data`. Host cu dang dung legacy/anonymous volume phai chay `observability/scripts/migrate-redis-volume.sh check`, sau do thuc hien maintenance migration co confirmation; khong xoa source/rollback volume truoc khi restart verification pass.
